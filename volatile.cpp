...
*  @Last Modified  : 2017/10/15 12:36:46
********************************************************************************
*/
...
/*
 * volatile 三种使用的举例：
 * 1.线程中访问的全局变量应该声明成volatile
 * 2.中断函数中访问的全局变量应该声明成volatile
 * 3.使用立即数作为某个外设地址时，应该使用volatile关键字。否则编译器可能优化成访问
 * CPU的缓存寄存器，而不是立即访问外设的实现物理地址。
 */
